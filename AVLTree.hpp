...
public:
    ...
    void remove(T& data, unsigned long long int& phone, int& group) {while (root->search(data, phone, group) != nullptr) {remove(root->search(data, phone, group));}}
...
bool AVLTree<T>::contains( T& data,  unsigned long long int& phone,  int& group)  {
    if (search(data,phone,group) == nullptr) return false;
    return true;
}
...
Node<T>* Node<T>::search(T& data, unsigned long long int &phone, int &group) {
    ...
    if (temp == nullptr) { return nullptr; }
        ...
        if (data == "" && phone == 0 && group != 0) {
            if (temp->group == group) { return temp; }
        }
        if (data != "" && phone != 0 && group == 0) {
            if (temp->data == data && temp->phone == phone) { return temp; }
        }
        if (data == "" && phone != 0 && group != 0) {
            if (temp->phone == phone && temp->group == group) { return temp; }
        }
        if (data != "" && phone == 0 && group != 0) {
            if (temp->data == data && temp->group == group) { return temp; }
        }
        if (data != "" && phone != 0 && group != 0) {
            if (temp->data == data && temp->phone == phone && temp->group == group) { return temp; }
        }
...
void Node<T>::print(T& data, unsigned long long int& phone, int& group) {
    ...
    if (data != "" && phone == 0 && group == 0) {
        if (temp->data == data) { cout << temp->data << " " << temp->phone << " " << temp->group << endl; }
    }
    if (data == "" && phone != 0 && group == 0) {
        if (temp->phone == phone) { cout << temp->data << " " << temp->phone << " " << temp->group << endl; }
    }
    if (data == "" && phone == 0 && group != 0) {
        if (temp->group == group) { cout << temp->data << " " << temp->phone << " " << temp->group << endl; }
    }
    if (data != "" && phone != 0 && group == 0) {
        if (temp->data == data && temp->phone == phone) { cout << temp->data << " " << temp->phone << " " << temp->group << endl; }
    }
    if (data == "" && phone != 0 && group != 0) {
        if (temp->phone == phone && temp->group == group) { cout << temp->data << " " << temp->phone << " " << temp->group << endl; }
    }
    if (data != "" && phone == 0 && group != 0) {
        if (temp->data == data && temp->group == group) { cout << temp->data << " " << temp->phone << " " << temp->group << endl; }
    }
    if (data != "" && phone != 0 && group != 0) {
        if (temp->data == data && temp->phone == phone && temp->group == group) { cout << temp->data << " " << temp->phone << " " << temp->group << endl; }
    }
...
void AVLTree<T>::printInOrder(Node<T>* node) const {
    ...
    if(getHeight(root)-getHeight(node)>10){return;}
