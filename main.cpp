...
int main() {
    ...
    while ((NS = accept(S,0,0))!=INVALID_SOCKET){
        ...
        if (line.find("delete") == 0) {
            ...
            iss >> tmp;//delete
            if (tmp != "delete") { cout << "err" << endl; continue; }
            tmp.clear();
            iss >> tmp;//where
            if (tmp != "where") { cout << "err" << endl; continue; }
            tmp.clear();
            ...
            for (auto block : foror) {
                ...
                for (auto element : forand) {
                    if (element.find("name")==0) {
                        ...
                        else { cout << "something wrong with name" << endl; }
                        continue;
                    }
                    if (element.find("phone")==0) {
                        ...
                        else { cout << "something wrong with phone" << endl; }
                        continue;
                    }
                    if (element.find("group")==0) {
                        ...
                        else { cout << "something wrong with group" << endl; }
                        continue;
                    }
        ...
        if (line.find("search") == 0) {
            ...
            iss >> tmp;//search
            if (tmp != "search") { cout << "err" << endl; continue; }
            tmp.clear();
            iss >> tmp;//where
            if (tmp != "where") { cout << "err" << endl; continue; }
            tmp.clear();
